"""
EXPERIMENTAL VERSION: Modified Weights
Testing: What happens when we double energy weight and halve genre weight?
"""
...
def score_song(user_prefs, song):
    """
    EXPERIMENTAL VERSION: Modified weights
    - Genre weight: 25 → 12.5 (halved)
    - Energy weight: 20 → 40 (doubled)
    ...
    # MODIFIED WEIGHTS (total still = 100)
    GENRE_WEIGHT = 12.5        # HALVED from 25
    MOOD_WEIGHT = 20           # UNCHANGED
    ENERGY_WEIGHT = 40         # DOUBLED from 20
    VALENCE_WEIGHT = 15        # UNCHANGED
    TEMPO_WEIGHT = 7.5         # Reduced to keep total = 100
    DANCEABILITY_WEIGHT = 2.5  # Reduced to keep total = 100
    ACOUSTICNESS_WEIGHT = 2.5  # Reduced to keep total = 100
    ...
    # Numerical matching: Energy (DOUBLED IMPORTANCE)
    energy_dist = abs(user_prefs['target_energy'] - song['energy'])
    energy_score = ENERGY_WEIGHT * (1 - energy_dist)
    score += energy_score
    reasons.append(f"Energy similarity: {energy_score:.1f} pts (song: {song['energy']:.2f}, target: {user_prefs['target_energy']:.2f}) [DOUBLED WEIGHT]")
